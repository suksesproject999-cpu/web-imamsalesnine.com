...
async function parse(event){
  ...
  const req=new Readable();req.push(b);req.push(null);req.headers={...(event.headers||{}),"content-length":b.length};req.method=event.httpMethod;req.url="/";
...
function imageData(files){
  const x=files?.image;if(!x)return null;const f=Array.isArray(x)?x[0]:x;if(!f?.filepath)return null;
...
function stateFrom(memory,productMemory,message){
  ...
  if(pm.length){const p=pm[pm.length-1];st.activeProduct={name:p.nama||p.name||"",sku:p.sku||"",product_id:p.product_id||""};}
  ...
  const q=message.replace(/\b(harga|stok|foto|gambar|spek|spec|spesifikasi|varian|detail|produk|nine|berapa|bro)\b/gi," ").replace(/\s+/g," ").trim();
  const exact=K.exactProduct(q||message);
  if(exact) st.activeProduct={name:exact.name,sku:exact.sku,product_id:exact.product_id};
  return st;
}
function publicState(st,p){
  const x=p||(st.activeProduct?K.byIdentity(st.activeProduct.sku||st.activeProduct.name||st.activeProduct.product_id):null);
  return {activeProduct:x?{name:x.name,nama:x.name,sku:x.sku,product_id:x.product_id,gambar:x.visual?.main||""}:st.activeProduct,vehicle:st.vehicle};
}
...
function direct(route){
  ...
  if(route.type==="product_detail") return [p.name,p.brand&&`Brand: ${p.brand}`,p.category&&`Kategori: ${p.category}`,p.sku&&`SKU: ${p.sku}`,p.price&&`Harga: ${p.price}`,p.stock!==""&&`Stok: ${p.stock}`,p.description&&`Deskripsi: ${p.description}`].filter(Boolean).join("\n");
  return null;
}
function isTime(q){return /\b(jam berapa|sekarang jam|pukul berapa|waktu sekarang)\b/i.test(q);}
function timeText(iso,tz){
  try{const d=iso?new Date(iso):new Date();const t=new Intl.DateTimeFormat("id-ID",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false,timeZone:tz||undefined}).format(d);const dt=new Intl.DateTimeFormat("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric",timeZone:tz||undefined}).format(d);return `Sekarang pukul ${t}${tz?` (${tz})`:""}, ${dt}.`;}catch{return null;}
}
function jsonResp(code,body){return {statusCode:code,headers:{"Content-Type":"application/json","Cache-Control":"no-store"},body:JSON.stringify(body)};}
...
exports.handler=async(event)=>{
  try{
    ...
    if(fast)return jsonResp(200,{reply:fast,image:null,route:route.type,source:Router.sourceFor(route),usedAI:false,state:publicState(state,route.product)});

    const ai=await AI.respond({message,route,state,memory,image:img,admin});
    return jsonResp(200,{reply:ai.reply,image:null,route:route.type,source:Router.sourceFor(route),usedAI:true,usedWeb:ai.usedWeb,sources:ai.sources,state:publicState(state,route.product)});
  }catch(e){
    console.error("NEXAI V7 ERROR:",e);
